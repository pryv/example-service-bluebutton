...
describe('Storage', function () {
  ...
  it('should create a zip file and delete all the user\'s info on db', function (done) {
    ...
    var backupPath = path.normalize(__dirname + '/../../backup/');
    ...
    async.series([
      ...
      function createBackup(stepDone) {
        mkdirp(backupPath + credentials.username);
        stepDone();
      },
      function createZip(stepDone) {
        db.createZip(credentials.username);
        should.equal(fs.existsSync(downloadPath + zipFile), true);
        stepDone();
      },
      function checkZip(stepDone) {
        var ls = fs.readdirSync(downloadPath);
        should.equal(ls.find(function (file) {
          return file === zipFile;
        }), zipFile);
        stepDone();
      },
